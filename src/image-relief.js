function blur(field, w, h, passes = 1) {
  let current = field;
  for (let pass = 0; pass < passes; pass++) {
    ...
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
      ...
      for (let oy = -2; oy <= 2; oy++) for (let ox = -2; ox <= 2; ox++) {
        ...
        const ww = ox === 0 && oy === 0 ? 4 : (Math.abs(ox) + Math.abs(oy) <= 1 ? 2 : 1);
...
function morph(mask,w,h,passes=2){
  let current=mask;
  for(let pass=0;pass<passes;pass++){
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){
      ...
      for(let oy=-1;oy<=1;oy++)for(let ox=-1;ox<=1;ox++){
        ...
        if(sx>=0&&sy>=0&&sx<w&&sy<h&&current[sy*w+sx])n++;
      }
      // Majority filtering removes isolated square pixels while keeping the principal silhouette.
      next[y*w+x]=n>=5?1:0;
    }
    current=next;
  }
...
export function analyzeRasterImage(img,maxSide=420){
  const ratio=img.width/img.height;
  const w=ratio>=1?maxSide:Math.max(96,Math.round(maxSide*ratio));
  const h=ratio>=1?Math.max(96,Math.round(maxSide/ratio)):maxSide;
  ...
  for(let i=0;i<mask.length;i++){
    if(hasAlphaCutout) mask[i]=alpha[i]>.12?1:0;
    else {const d=Math.abs(lum[i]-bgLum);mask[i]=alpha[i]>.08&&(bgLum<.72||d>.045||lum[i]<.92)?1:0}
  }
  mask=morph(mask,w,h,2);mask=largestComponent(mask,w,h);
  ...
  for(let y=0;y<ch;y++)for(let x=0;x<cw;x++){const src=(y+minY)*w+x+minX;croppedMask[y*cw+x]=mask[src];croppedLum[y*cw+x]=lum[src]}
  // Two weighted passes suppress JPEG/PNG stair stepping before height generation.
  const smooth=blur(croppedLum,cw,ch,2),edge=new Float32Array(cw*ch);
  for(let y=1;y<ch-1;y++)for(let x=1;x<cw-1;x++){
    const gx=sample(smooth,cw,ch,x+1,y)-sample(smooth,cw,ch,x-1,y),gy=sample(smooth,cw,ch,x,y+1)-sample(smooth,cw,ch,x,y-1);
    edge[y*cw+x]=Math.min(1,Math.sqrt(gx*gx+gy*gy)*4.2);
  }
